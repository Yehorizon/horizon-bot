...
from telegram.ext import Application, CommandHandler, CallbackQueryHandler, MessageHandler, C
from telegram.constants import ParseMode, ChatAction
TELEGRAM_TOKEN = os.environ.get('TELEGRAM_TOKEN')
ANTHROPIC_API_KEY = os.environ.get('ANTHROPIC_API_KEY')
client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY)
REGIONS = {
'all': ('Всі регіони', 'geopolitics international relations breaking news today 2
'ukraine': ('Україна', 'europe': 'Ukraine war conflict diplomacy news today 2026'),
('Європа / НАТО', 'Europe EU NATO security politics news today 2026'),
...
'china': ('Китай', 'China geopolitics Taiwan foreign policy news today 'mideast': ('Близький Схід', 'Middle East conflict Israel Iran diplomacy news today 202
2026')
}
QUICK_QUESTIONS = [
...
max_tokens=1000,
system='You are a geopolitical news aggregator. Find 5 news items and return ONLY a J
messages=[{'role': 'user', 'content': 'Find 5 current geopolitical news: ' + region_q
...
model='claude-sonnet-4-20250514',
max_tokens=900,
system='You are a senior geopolitical analyst. Answer in Ukrainian. Be specific and a
...
model='claude-sonnet-4-20250514',
max_tokens=1000,
system='You are a senior geopolitical analyst. Answer structured and analytically in
...
buttons = [[InlineKeyboardButton(label, callback_data='region_' + key)] for key, (label,
...
buttons = [[InlineKeyboardButton(q[:60], callback_data='quick_' + str(i))] for i, q in en
buttons.append([InlineKeyboardButton('Назад', callback_data='menu_main')])
return InlineKeyboardMarkup(buttons)
...
if not articles:
await query.edit_message_text('Не вдалося завантажити новини.', reply_markup=back
return
...
loop = asyncio.get_event_loop()
analysis = await loop.run_in_executor(None, analyze_event, article['title'], article[
text = article['title'] + '\n' + article.get('source', '') + '\n\n' + analysis
await query.edit_message_text(
text[:4096],
...
await update.message.reply_text('Скористайся /start для навігації', reply_markup=main
return
